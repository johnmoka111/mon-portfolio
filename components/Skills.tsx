...
const FirebaseIcon = () => (
  <svg viewBox="0 0 24 24" className="h-10 w-10 sm:h-12 sm:w-12" fill="currentColor">
    <path d="M12 2.69l5.66 5.5-2.23-9.5a.33.33 0 0 0-.57-.16l-2.86 4.16zM3.6 15.42l9.08-8.8 1.34-5.7a.33.33 0 0 0-.57-.16L3.6 15.42zm16.8-5.32l-1.34-5.7-9.08 8.8 1.75 9.87a.33.33 0 0 0 .57.16l8.1-13.13z"/>
    <path d="M1.35 18l8.83-8.61c.47-.46 1.22-.46 1.69 0L20.65 18 12 23.31c-.47.29-1.22.29-1.69 0L1.35 18z"/>
  </svg>
);
...
export default function Skills() {
  ...
  const skillsList: SkillItem[] = [
    ...
    { key: "firebase", svgIcon: <FirebaseIcon /> },
  ...
  return (
    <section id="skills" className="py-24 bg-slate-50 dark:bg-slate-950 transition-colors duration-300 relative">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-kivuBlue/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <motion.div
          ...
          className="flex flex-wrap items-center justify-center gap-6 sm:gap-8 max-w-3xl mx-auto"
        >
          {skillsList.map((skill) => (
            <motion.div
              ...
              className="group relative flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 bg-white dark:bg-slate-900 border border-gray-200/80 dark:border-slate-800/80 rounded-3xl shadow-sm hover:shadow-lg hover:shadow-kivuBlue/5 dark:hover:shadow-kivuBlue/10 hover:border-kivuBlue/50 dark:hover:border-kivuBlue/50 transition-all duration-300 cursor-default"
            >
              {/* Tooltip */}
              <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-950 dark:bg-white text-white dark:text-slate-950 text-xs font-bold py-1.5 px-3 rounded-lg shadow-md whitespace-nowrap opacity-0 scale-90 group-hover:opacity-100 group-hover:scale-100 pointer-events-none transition-all duration-200 z-30">
                {t(`list.${skill.key}`)}
                {/* Arrow */}
              ...
              {/* Icon Container */}
              <div className="text-gray-700 dark:text-gray-300 group-hover:text-kivuBlue dark:group-hover:text-blue-400 transition-colors duration-300">
                {skill.svgIcon}
              </div>
            </motion.div>
          ))}
